...
function handleMouseWheel(e) {
  ...
  const contentWidth = content.value?.offsetWidth
  /**
   * @wheelDelta parallel scroll value >0： move right  <0: move left
   * @translateX ContenttranslateXvalue
...
function handleScroll(x, width) {
  ...
  // when x less thancanvisible range minimumvaluetime
  if (x < -translateX.value + 150) {
    translateX.value = -(x - 150)
    resetTranslateX(wrapperWidth, contentWidth)
  }

  // when x greater thancanvisible range maximumvaluetime
