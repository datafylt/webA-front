...
// * BackendRouterelatedfunction
// based onBackendpassedDatabuildoutputFrontendRoute
...
function buildRoutes(routes = []) {
  return routes.map((e) => {
    ...
    if (e.children && e.children.length > 0) {
      ...
      // noSub Menu，CreateoneaDefaultchildRoute
      route.children.push({
        name: `${e.name}Default`,
        path: '',
...
export const usePermissionStore = defineStore('permission', {
  ...
  actions: {
    async generateRoutes() {
      const res = await api.getUserMenu() // CallAPIGetBackendpassedMenuRoute
      this.accessRoutes = buildRoutes(res.data) // HandletoFrontendRouteformat
